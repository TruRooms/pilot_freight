...
module PilotFreight
  class XML
    ...
    def self.encode_and_validate(hash)
      encoded = self.encode(hash)
      errors = self.validate(encoded)
      raise PilotFreight::InvalidXML, "Invalid XML: #{errors}" if errors.length > 0
      return encoded
    end
    ...
    def self.validate(xml)
      dtd = Nokogiri::XML::DTD.new('dtd', Nokogiri::XML::Document.parse(File.open(File.join(File.dirname(__FILE__), 'support/shipment.dtd'))))
    ...
    def self.to_open_struct(hash)
      return OpenStruct.new if hash.nil? || hash.length == 0
      output = hash.each do |k,v|
        if v.is_a? Hash
